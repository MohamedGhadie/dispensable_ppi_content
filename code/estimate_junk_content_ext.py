...
def main():
    ...
    # directory to save extended processed data specific to interactome
    interactomeOutDir_ext = outDir / interactome_name / 'ext'
    
    # figure directory
    figDir = Path('../figures') / interactome_name / 'ext'
    if consider_exp_QuasiNull_perturbs:
        figDir = figDir / 'exp_QuasiNull_perturbs_considered'
    
    # figure directory for experimental results
    expFigDir = Path('../figures/experiment')
    ...
    if not interactomeOutDir_ext.exists():
        os.makedirs( interactomeOutDir_ext )
    if not figDir.exists():
        os.makedirs( figDir )
    if not expFigDir.exists():
...
#     diseasePerturbs = diseasePerturbs[diseasePerturbs["perturbations"].apply(lambda x: sum(x > 0) < 2)].reset_index(drop=True)   
    ...
    junkPPIFile = interactomeOutDir_ext / 'fraction_junk_PPIs.pkl'
    ...
    for approach in ['geometrybased_prediction_perMut',
                     ...
                     'experiment_perPPI']:
        allresults = {}
        allresultsFile = interactomeOutDir_ext / ('edgetic-prob_' + approach + '.pkl')
        ...
        # Probability for edgetic mutations to be mildly deleterious
        pM_E = pE_M * pM / pE
        
        # Probability for edgetic mutations to be strongly detrimental
        pS_E = pE_S * pS / pE
        
        allresults['P(N|E)'] = pN_E
        allresults['P(M|E)'] = pM_E
        allresults['P(S|E)'] = pS_E
        
        pN_E_results.append( 100 * pN_E )
        pM_E_results.append( 100 * pM_E )
        pS_E_results.append( 100 * pS_E )
        ...
        print( 'Probability for edgetic mutations to be effectively neutral P(N|E) = %.1f %%' % (100 * pN_E) )
        print( 'Probability for edgetic mutations to be mildly deleterious P(M|E) = %.1f %%' % (100 * pM_E) )
        print( 'Probability for edgetic mutations to be strongly detrimental P(S|E) = %.1f %%' % (100 * pS_E) )
        
        # calculate 95% confidence interval    
        if computeConfidenceIntervals:
            n_N, n_M = numMut
            k_obs_N, k_obs_M = numEdgetic
            for prob, p in zip(('P(N|E)', 'P(M|E)'), (pN_E, pM_E)):
                if prob is 'P(N|E)':
                    pRR_lower, pRR_upper = proportion_ratio_CI (k_obs_M, n_M, k_obs_N, n_N)
                    coef = pM / pN
                elif prob is 'P(M|E)':
                    pRR_lower, pRR_upper = proportion_ratio_CI (k_obs_N, n_N, k_obs_M, n_M)
                    coef = pN / pM
                if np.isnan( pRR_lower ) or np.isnan( pRR_upper ):
                    print( '%.1f%% confidence interval for %s = NA' % (CI, prob) )
                else:
                    p_lower = 1 / ( pRR_upper * coef + 1 )
                    p_upper = 1 / ( pRR_lower * coef + 1 )
                    print( '%.1f%% confidence interval for %s = (%f, %f)' 
                            % (CI, prob, 100 * p_lower, 100 * p_upper) )
                    allresults[prob + '_CI'] = [p - p_lower, p_upper - p]
            allresults['P(S|E)_CI'] = [0, 0]
            if 'P(N|E)_CI' in allresults:
                pN_E_bounds.append( [100 * p for p in allresults['P(N|E)_CI']] )
            else:
                pN_E_bounds.append( [0, 0] )
            if 'P(M|E)_CI' in allresults:
                pM_E_bounds.append( [100 * p for p in allresults['P(M|E)_CI']] )
            else:
                pM_E_bounds.append( [0, 0] )
            if 'P(S|E)_CI' in allresults:
                pS_E_bounds.append( [100 * p for p in allresults['P(S|E)_CI']] )
            else:
                pS_E_bounds.append( [0, 0] )
        with open(allresultsFile, 'wb') as fOut:
    ...
    # split prediction results from experiment results
    predProb, predProb_ddg, expProb = zip(pN_E_results[:3], pM_E_results[:3], pS_E_results[:3])
    predCI, predCI_ddg, expCI = zip(pN_E_bounds[:3], pM_E_bounds[:3], pS_E_bounds[:3])
    
    # plot fractions of edgetic mutations that are effectively neutral, mildly deleterious, 
    # or strongly detrimental from geometry-based predictions per mutation
    if computeConfidenceIntervals:
        upper = [ p + upper for p, (lower, upper) in zip(predProb, predCI) ]
        maxY = max( upper )
    else:
        maxY = max( predProb )
    ...
    maxY = 100
    bar_plot(predProb,
             predCI if computeConfidenceIntervals else [ 0 ],
             xlabels = ('Effectively\nneutral',
                        'Mildly\ndeleterious',
                        'Strongly\ndetrimental'),
             ylabels = list(np.arange(0, maxY + 20, 20)),
             ylabel = 'Fraction among edgetic mutations (%)',
             colors = 'white',
             fmt = 'k.',
             capsize = 10 if computeConfidenceIntervals else 0,
             msize = 11,
             ewidth = 1,
             #ecolors = [interactome_color, 'orangered'],
             ecolors = 'black',
             fontsize = 14,
             xlim = [0.8, 3.1],
             ylim = [0, maxY],
             ...
             show = showFigs,
             figdir = figDir,
             figname = 'all-edgetic_geometryPred_perMut')
    
    # plot fractions of edgetic mutations that are effectively neutral, mildly deleterious, 
    # or strongly detrimental from physics-based predictions per mutation
    if computeConfidenceIntervals:
        upper = [ p + upper for p, (lower, upper) in zip(predProb_ddg, predCI_ddg) ]
        maxY = max( upper )
    else:
        maxY = max( predProb_ddg )
    maxY = 20 * np.ceil( maxY / 20 )
    maxY = 100
    bar_plot(predProb_ddg,
             predCI_ddg if computeConfidenceIntervals else [ 0 ],
             xlabels = ('Effectively\nneutral',
                        'Mildly\ndeleterious',
                        'Strongly\ndetrimental'),
             ylabels = list(np.arange(0, maxY + 20, 20)),
             ...
             show = showFigs,
             figdir = figDir,
             figname = 'all-edgetic_physicsPred_perMut')
        
    # plot fractions of edgetic mutations that are effectively neutral, 
    # mildly deleterious, and strongly detrimental from experiments
    if computeConfidenceIntervals:
        upper = [ p + upper for p, (lower, upper) in zip(expProb, expCI) ]
        maxY = max( upper )
    else:
        maxY = max( expProb )
    maxY = 20 * np.ceil( maxY / 20 )
    maxY = 100
    bar_plot(expProb,
             expCI if computeConfidenceIntervals else [ 0 ],
             xlabels = ('Effectively\nneutral',
                        'Mildly\ndeleterious',
                        'Strongly\ndetrimental'),
             ylabels = list(np.arange(0, maxY + 20, 20)),
