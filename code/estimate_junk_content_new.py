...
from mutation_interface_edgotype import (mutation_PPI_interface_perturbations,
                                         energy_based_perturb_prediction2)
...
def main():
    ...
    if consider_exp_QuasiNull_perturbs:
        figDir = Path('../figures') / interactome_name / 'exp_QuasiNull_perturbs_considered_new'
    else:
        figDir = Path('../figures') / (interactome_name + '_new')
...
#     diseasePerturbs = diseasePerturbs[diseasePerturbs["perturbations"].apply(lambda x: sum(x > 0) < 2)].reset_index(drop=True)   
    ...
    naturalMutationsDDG = read_mutation_ddg(interactomeOutDir / 'nondisease_mutations_onchains_ddg_7.txt')
    diseaseMutationsDDG = read_mutation_ddg(interactomeOutDir / 'disease_mutations_onchains_ddg_7.txt')
    ...
    if ddgMutationPerturbFile.is_file():
        ...
        perturbations, knownDDG, unknownDDG = energy_based_perturb_prediction2 (naturalPerturbs,
                                                                               naturalMutationsDDG,
                                                                               ddgCutoff)
        perturbations = perturbations.apply(lambda x: np.array([p for p in x if not np.isnan(p)]))
        perturbations = perturbations[perturbations.apply(len) > 0]
        ...
        perturbations, knownDDG, unknownDDG = energy_based_perturb_prediction2 (diseasePerturbs,
                                                                               diseaseMutationsDDG,
                                                                               ddgCutoff)
        perturbations = perturbations.apply(lambda x: np.array([p for p in x if not np.isnan(p)]))
        perturbations = perturbations[perturbations.apply(len) > 0]
